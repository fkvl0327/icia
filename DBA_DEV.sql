...
/* 전체 상품중 현재 판매자가 없어 판매되지 않는 상품 정보 조회 
   SELECT * FROM GO;  --> 판매할 수 있는 전체 상품
   SELECT * FROM SA;  --> 판매 중 인 상품
   A       1  A  B  C     1A  3A
   B       2  C  D
   C       3  A  E
   D
...
/* 전체 상품 중 판매되는 상품의 판매자 정보 조회 
    --------------------------------------------------------
      상품코드   상품명   판매자코드  판매자명    판매가격 재고수량
    --------------------------------------------------------
*/
;
SELECT COALESCE(SA.SA_GOCODE, GO.GO_CODE) AS "GCODE_NOW",
       GO.GO_NAME AS "GNAME",
       COALESCE(SA.SA_SECODE, N'NONE') AS "SCODE",
       COALESCE(SE.SE_NAME, N'NONE') AS "SNAME",
       COALESCE(SA.SA_PRICE, 0) AS "PRICE",
...
/* 1. 특정 판매자의 판매정보 리스트 조회 
    ...
    DEVELOPER : 
 */
 판매자코드    판매자이름 상품코드 상품이름 판매가격 판매개수 매출(판매가격*판매개수) 
 OD_SASECODE SE_NAME OD_SAGOCODE GO_NAME SA_PRICE OD_QUANTITY
 SE_CODE             SA_GOCODE
 SA_SECODE           GO_CODE
 Oracle 접속
...
4~6 윤
 ...
 FROM OD INNER JOIN SA ON OD.OD_SASECODE = SA.SA_SECODE AND OD.OD_SAGOCODE = SA.SA_GOCODE
         INNER JOIN GO ON OD.OD_SAGOCODE = GO.GO_CODE
         INNER JOIN SE ON OD.OD_SASECODE = SE.SE_CODE
 GROUP BY OD.OD_SASECODE, SE.SE_NAME, OD.OD_SAGOCODE, GO.GO_NAME, SA.SA_PRICE,
        OD.OD_QUANTITY, OD.OD_ORDATE, SA.SA_PRICE * OD.OD_QUANTITY; -- 100개
 ...
 SELECT SA.SA_SECODE AS "SCODE",
        ...
        GO.GO_NAME AS "GNAME",
        SA.SA_PRICE AS "PRICE"
 FROM SA INNER JOIN GO ON SA_GOCODE = GO_CODE
         INNER JOIN SE ON SA_SECODE = SE_CODE
 GROUP BY SA.SA_SECODE, SA.SA_GOCODE, GO.GO_NAME, SE.SE_NAME, SA.SA_PRICE;
 -- 출력된 행 70개
...
SELECT SE_CODE, SE_NAME
FROM SE
WHERE SE_CODE IN (SELECT SE_CODE FROM SE MINUS SELECT OD_SASECODE FROM OD);
...
-- JOIN 쓸 경우 : 단, FK가 직접 연결된 SA를 통해서 연결하는 것이 좋다
-- (SA를 연결하면 SA값 INSERT 필요)
INSERT INTO SA(SA_SECODE, SA_GOCODE, SA_PRICE, SA_STOCK)
VALUES('1000812345', '1001', 1500, 100);
COMMIT;

SELECT SE_CODE, SE_NAME
FROM SE LEFT OUTER JOIN OD ON SE_CODE = OD_SASECODE
WHERE OD_ORMMID IS NULL; -- OD 안에서 어느 칼럼을 조건으로 삼아도 됨
-- SE에는 있고 OD에는 값이 없는 조건으로 조회한다는 의미

 /* 6. 판매실적이 있는 판매자 정보 조회 */
SELECT SE.SE_CODE,
       SE.SE_NAME
FROM SE INNER JOIN OD ON SE.SE_CODE = OD.OD_SASECODE
...
GRANT ALL ON MEMBERS TO yun, sol, hyeonha, hong;

EXECUTE MEMBERJOIN('MM_12345', '1234', '테스트', '01033332222');

EXECUTE MEMBERUPDATE('9999', 'MM_12345');

SELECT * FROM MM;

GRANT ALL ON SALES TO yun, sol, hyeonha, hong;

GRANT ALL ON SELLER TO yun, sol, hyeonha, hong;
GRANT ALL ON GOODS TO yun, sol, hyeonha, hong;
GRANT ALL ON ORDERDETAIL TO yun, sol, hyeonha, hong;
GRANT ALL ON ORDERS TO yun, sol, hyeonha, hong;

-- SELECT
VAR REQUEST_DATA REFCURSOR;
EXEC SEARCH_PERFORMANCE(:REQUEST_DATA);
PRINT REQUEST_DATA;

SELECT * FROM USER_SOURCE WHERE TEXT LIKE '%SEARCH_PERFORMANCE%';
...
SELECT SA.SA_SECODE AS "SCODE",
 ...
 GRANT EXECUTE ON V_P_SELLERSINFO TO SOL, HYEONHA, HONG;

CREATE OR REPLACE PROCEDURE MEMBERJOIN
( MID IN MM.MM_ID%TYPE,
  MPWD IN MM.MM_PASSWORD%TYPE,
  MNAME IN MM.MM_NAME%TYPE,
  MPHONE IN MM.MM_PHONE%TYPE
)
IS
BEGIN
  INSERT INTO MM(MM_ID, MM_PASSWORD, MM_NAME, MM_PHONE, MM_STATE)
  VALUES(MID, MPWD, MNAME, MPHONE, 'A');
  COMMIT;
END MEMBERJOIN;
...
IS
BEGIN
  UPDATE MM SET MM_PASSWORD=MPWD
  WHERE MM_ID= MID;
  COMMIT;
END MEMBERUPDATE;

CREATE OR REPLACE PROCEDURE SEARCH_PERFORMANCE
( REQUEST_DATA OUT SYS_REFCURSOR )
IS
BEGIN
 ...
 FROM SA LEFT OUTER JOIN OD ON SA.SA_SECODE = OD.OD_SASECODE AND SA.SA_GOCODE = OD.OD_SAGOCODE
         ...
         INNER JOIN GO ON SA.SA_GOCODE = GO.GO_CODE
 GROUP BY SA.SA_SECODE, SA.SA_GOCODE, SE.SE_NAME, GO.GO_NAME;
 END SEARCH_PERFORMANCE;

CREATE OR REPLACE PROCEDURE INS_ORDERS
 ( MID      IN  "OR".OR_MMID%TYPE,
   MDATE    OUT NCHAR)
IS
BEGIN
    INSERT INTO "OR"(OR_MMID, OR_DATE, OR_STATE) 
    VALUES(MID, SYSDATE, 'I');
    SELECT TO_CHAR(MAX(OR_DATE), 'YYYYMMDDHH24MISS') INTO MDATE FROM "OR" 
    WHERE OR_MMID = MID;
    COMMIT;
END INS_ORDERS;


CREATE OR REPLACE PROCEDURE V_P_SALESINFO
(SECODE             IN   SA.SA_SECODE%TYPE,
REQUEST_DATA      OUT     SYS_REFCURSOR
)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE V_P_SELLERSINFO
(SCODE             IN   SA.SA_SECODE%TYPE,
REQUEST_DATA      OUT     SYS_REFCURSOR
)
IS
BEGIN
        OPEN REQUEST_DATA FOR
        SELECT * FROM SELLER_HAS_OD WHERE SCODE = SE_CODE;
END V_P_SELLERSINFO;
...
/* TRIGGER 
    ...
    [BEFORE|AFTER]
    [INSERT|UPDATE|DELETE] ON [TAB_NAME]
    [FOR EACH ROW]
    [WHEN [CONDITION]]
*/

-- 주문이 체결되면 판매된 상품의 재고 감소
CREATE OR REPLACE TRIGGER UPD_STOCK
    AFTER INSERT ON OD FOR EACH ROW
BEGIN
    UPDATE SA SET SA_STOCK = SA_STOCK - :NEW.OD_QUANTITY 
    WHERE SA_SECODE = :NEW.OD_SASECODE AND SA_GOCODE = :NEW.OD_SAGOCODE;  
END UPD_STOCK;

/* TRIGGER 작업 후 실행 */
-- ORDERS에 RECORD 생성 PROC_NAME : INS_ORDERS
VAR MDATE NVARCHAR2;   
EXEC INS_ORDERS('MM8', :MDATE)
PRINT MDATE;

SELECT * FROM "OR";

-- ORDERS DETAIL.주문 상품 등록
INSERT INTO OD(OD_ORMMID, OD_ORDATE, OD_SAGOCODE, OD_SASECODE, OD_QUANTITY, OD_STATE) 
VALUES('MM8', TO_DATE('20201218151155', 'YYYYMMDDHH24MISS'),'2001', '1000112345', 10, 'I');
INSERT INTO OD(OD_ORMMID, OD_ORDATE, OD_SAGOCODE, OD_SASECODE, OD_QUANTITY, OD_STATE) 
VALUES('MM8', TO_DATE('20201218151155', 'YYYYMMDDHH24MISS'),'2002', '1000112345', 10, 'I');
INSERT INTO OD(OD_ORMMID, OD_ORDATE, OD_SAGOCODE, OD_SASECODE, OD_QUANTITY, OD_STATE) 
VALUES('MM8', TO_DATE('20201218151155', 'YYYYMMDDHH24MISS'),'2003', '1000112345', 10, 'I');

-- 결재
UPDATE "OR" SET OR_STATE = 'C' WHERE OR_MMID = 'MM8' AND OR_DATE = '20201218151155';
UPDATE OD SET OD_STATE = 'C' WHERE OD_ORMMID = 'MM8' AND OD_ORDATE = '20201218151155';
SELECT * FROM "OR" WHERE OR_MMID = 'MM8';
-- TRANSACTION 종료
COMMIT;
ROLLBACK;
...
SELECT * FROM MM;
SELECT * FROM AL WHERE AL_ID = 'MM1';

INSERT INTO AL(AL_ID, AL_TIME, AL_TYPE) VALUES ('MM1', DEFAULT, 오윤 1);
ROLLBACK;
...
SELECT * FROM DBA5.MINFO WHERE MID='MM1' AND ALTYPE=1;

SELECT * FROM DBA5.MINFO WHERE MID='MM1';
select * from MM;

SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'YUN';

GRANT SELECT ON MINFO TO YUN, SOL, HYEONHA, HONG;
...
-- 이미지 컬럼에 유니크 제약 조건 추가
ALTER TABLE SALES
ADD CONSTRAINT SA_IMAGE_UK UNIQUE(SA_IMAGE);

-- DEV
SELECT * FROM GOODSINFO;

/* IMAGE 경로 및 IMAGE 파일 등록 상품코드_판매자코드.JPG */
UPDATE SA SET SA_IMAGE = SA_GOCODE || '_' || SA_SECODE || '.jpg';
COMMIT;

/* 2021-01-06 (수) */

-- 상세정보 가져오기
SELECT * FROM GOODSINFO WHERE GOCODE='1001' AND SECODE='1000812345';
-- 뷰를 만들 때 각 테이블의 PK를 집어넣으라고 하는 이유
...
-- 상세 이미지도 입력할 텐데, 몇 개나 이미지가 입력될지 미리 알지 못할 수 있고 정해놓는 것은 비효율적이므로
...
-- DBA :: 판매상품테이블 변경
 ...
 CREATE PUBLIC SYNONYM SI FOR HOONZZANG.SAIMAGE;   
 ...
 --6. 상품 상세 이미지
 SELECT SI_SAGOCODE, SI_SASECODE, SUBSTR(SI_SAIMAGE, 1, 15) || '_B.jpg';
 
-- DEV
SELECT * FROM SI;

SELECT * FROM DBA5.GOODSINFO;

SELECT * FROM GOODSINFO ORDER BY GOCODE, SECODE;

/* 1월 7일자 VIEW 수정: SI 테이블의 내용도 VIEW 하나에서 볼 수 있게 REPLACE */
CREATE OR REPLACE VIEW GOODSINFO
AS
SELECT  SA.SA_GOCODE AS GOCODE,
        GO.GO_NAME AS GONAME,
...
SELECT TO_CHAR(MAX(OR_DATE), 'YYYYMMDDHH24MISS') AS ORDATE FROM "OR" WHERE OR_MMID='fkvl' AND OR_STATE='I';
SELECT * FROM OD WHERE OD_ORMMID = 'fkvl';
SELECT * FROM "OR" WHERE OR_MMID = 'fkvl';
SELECT * FROM OD;

INSERT INTO OD(OD_ORMMID, OD_ORDATE, OD_SAGOCODE, OD_QUANTITY, OD_STATE, OD_SASECODE)
VALUES('fkvl', '20210111183924', '1003', 1, 'I', '1000312345');

SELECT * FROM BA WHERE BA_MMID='fkvl';
ROLLBACK;

/* 장바구니 조회
- DBA: CREATE VIEW > GRANT SELECT ON VIEW > CREATE PUBLIC SYNONYM -- 직접 해보기><
- DEV: SELECT * FROM BL;
*/
...
CREATE OR REPLACE VIEW ORDERLIST
AS
...
CREATE PUBLIC SYNONYM OL FOR DBA5.ORDERLIST;


 GRANT SELECT, INSERT, UPDATE ON OL TO yun, sol, hong, hyeonha;
 
 SELECT * FROM OL WHERE MID='fkvl';
 
 DROP VIEW ORDERLIST;
 
 SELECT * FROM GOODSINFO;
 
 SELECT GOCODE, GONAME, SECODE, SENAME, PRICE, LIMAGE FROM GOODSINFO WHERE GOCODE = '1001' AND SECODE = '1000312345';
