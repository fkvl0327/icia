...
public class Authentication {
	...
	private Action loginCtl(HttpServletRequest req) {
		...
		boolean tran = false;
		boolean actionType = false; // 로그인 성공이든 실패든 사용자에게 문구를 보여주고자 하므로 req에 값을 저장 후 dispatcher방식으로 페이지를 이동해야 한다
			...
			if (this.isActive(auth)) {
				if (this.isAccess(auth)) {
					if (this.insAccessLog(auth)) {
						...
						session.setAttribute("accessInfo", session);
						
						list = this.searchMemberInfo(auth);
						tran = true;
						page = "index.jsp";
						msg = "";
						mInfo = list.get(0).getmId() + "(" + list.get(0).getmName() + ")님은 " + list.get(0).getAccessTime() + "에 로그인하셨습니다.";
					}else {
